...
version: alpha
name: Cartra
description: Cartra marketing UI system derived from the Cursor design kit — warm-cream editorial canvas (`#f7f7f4`), warm near-black ink (`#26251e`), single brand voltage **Cursor Orange** (`#f54e00`) for primary CTAs only, display at weight 400 with negative letter-spacing, hairline-only depth (no shadows), 80px section rhythm. CursorGothic substituted with Geist/Inter; JetBrains Mono on code and agent-workflow surfaces. Timeline pastels only inside in-product agent visualizations.
...
This file is the **canonical visual source of truth** for Cartra UI. All UI implementation and future UI work must follow it. Brand copy, SEO, and logo rules remain in `docs/marketing-context.md`. Audience and messaging personality in `.impeccable.md` still apply; **visual tokens and component treatment in this file supersede** the palette/type/layout direction previously recorded in `.impeccable.md`.

### Keep Cartra-specific (do not import from Cursor product UI)
...
### Explicit non-goals from the Cursor kit

- Do not adopt Cursor wordmark, IDE chrome branding, or “Download for macOS” patterns.
- Do not reintroduce cobalt/navy/peach as competing brand accents — `{colors.primary}` (#f54e00) is the only CTA voltage.

## Overview

Cursor's marketing site reads as a quietly-confident developer brand that believes in editorial calm over IDE-darkness. (Cartra adopts this visual system for marketing surfaces.) The base canvas is **warm cream** (`{colors.canvas}` — #f7f7f4) holding warm near-black ink (`{colors.ink}` — #26251e) for body and display alike. The single brand voltage is **Cursor Orange** (`{colors.primary}` — #f54e00) reserved for primary CTAs and the wordmark — used scarcely.
...
**`hero-agent-panel`** — White card (`{colors.surface-card}`) with hairline border, rounded `{rounded.lg}`. Desktop-only (`lg+`) beside hero copy. Layout: **50/50 grid on `lg`**, then capped copy column (`36rem`) + fluid animation column on `xl+`. Panel fills its column (`width: 100%`, height matched to copy block via `items-stretch`). Timeline pastels scoped to in-panel pills only.
