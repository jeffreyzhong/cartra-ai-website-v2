...
**`hero-agent-panel`** — White card (`{colors.surface-card}`) with hairline border, rounded `{rounded.lg}`. Desktop-only (`lg+`) beside hero copy. Timeline pastels scoped to in-panel pills only.
